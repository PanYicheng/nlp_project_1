...
class ContextEncoder(torch.nn.Module):
    """
    ContextEncoder
    Use a rnn to encode the input sequence into one context vector
...
class LanguageModel(torch.nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, emsize, nhid, nlayers,
        ...
        self.lockdrop = LockedDropout()
        self.input_embedding = nn.Embedding(ntoken, emsize)
        self.rnn = torch.nn.GRU(emsize, nhid, nlayers, dropout=dropoutrnn)
        ...
        if tie_weights:
            if nhid != emsize:
                raise ValueError('When using the tied flag, nhid must be equal to emsize')
            self.output_embedding = nn.Linear(nhid, ntoken, bias=False)
