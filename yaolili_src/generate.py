...
def evaluate(data, hidden, args):
    ...
    source, targets = get_batch(bdata, 0, args)
    ...
    # scoring the endings according to language models.
        ...
        with open(args.true_endings + '.score', 'w') as tof, open(args.fake_endings + '.score', 'w') as fof:
            ...
            while 1:
                ...
                tof.write(str(true_cond_cross_ent) + '\t' + str(true_cross_ent) + '\t' + str(tscore) + '\t' + str(
                    tscore < fscore) + '\n')
                tof.flush()
                fof.write(str(fake_cond_cross_ent) + '\t' + str(fake_cross_ent) + '\t' + str(fscore) + '\t' + str(
                    fscore < tscore) + '\n')
                fof.flush()
                if tscore < fscore:
                    correct += 1.0
                ...
                print('detailed scores: %f, %f, %f, %f.' % (
                true_cond_cross_ent, true_cross_ent, fake_cond_cross_ent, fake_cross_ent))
        ...
        with open(args.outf, 'w') as outf:  # , open('gold_4sent.txt', 'w') as gf:
            if args.task == 'cond_generate':
                data = corpus.tokenize(args.conditional_data,
                                       applydict=True).tolist()  # this is a list of ids corresponding to words from the word2idx dict
                nsent = 0
                while nsent < args.sents:
                    try:
                        idx = data.index(
                            eos_id)  # the only thing that breaks the while loop is if there are no more eos_ids
                    except:
                        break
                    ...
                    for i in range(args.words):
                        if i < cond_length:
                            ...
                            # clip the infinite value to a big value
                            word_weights[torch.isinf(word_weights)] = 1e8
